...
public class LifeView extends javax.swing.JPanel {
	...
	public LifeView(int c, int r) {
		...
		// initialize the arrays to start amount and create the smart
		// rectangles, set the parallel array to dead
		_theMobs = new SmartRectangle[c][r];
		_background = new SmartRectangle[c][r];
		_life = new boolean[c][r];

		// Set up the rectangles and the parallel array for live values
		// (Starting all dead)
		...
		for (int i = 0; i < _rows; i++) {
			for (int j = 0; j < _columns; j++) {
				...
				_background[i][j].setLocation(((i) * OFFSET) - 1,
						((j) * OFFSET) - 1);
			}
		}
	...
	// // Below is my Mouse Listener class, which deals with mouse events and
	// extends MouseAdapter
	private class MyMouseListener extends java.awt.event.MouseAdapter {

		public void mouseClicked(java.awt.event.MouseEvent e) {
			// When you click a point, toggle that points life value and
			// repaint.
			System.out.println("You have clicked point "
					+ (int) (e.getX() / OFFSET) + " , "
					+ (int) (e.getY() / OFFSET));
			if (_modify && (int) (e.getX() / OFFSET) < _rows
					&& (int) (e.getY() / OFFSET) < _columns) {
				_life[(int) (e.getX() / OFFSET)][(int) (e.getY() / OFFSET)] = !_life[(int) (e
						.getX() / OFFSET)][(int) (e.getY() / OFFSET)];
